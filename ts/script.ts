...
function CheckSetFrec(flag: boolean) {
    document.getElementById('rowts').innerHTML = "";
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Juicer && (<Juicer>productList[i]).set_frec) (<Juicer>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
            productList[i].Init();
...
class Juicer extends Product {
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public set_frec?: boolean) {
        super(id, name, price, description, inStock);
        this.Init();
    }
...
let productList: Product[] = [
    ...
    new FeltBoots(7, "Валенки1", 666.66, "Валенки великолепной работы слепого мастера Игната! В комплекте к валенкам идёт кокошник.", 2,
        [{ dimension: 20, color: Color.Pink, quantity: 1 },
