...
function CheckSetFrec(flag) {
    document.getElementById('rowts').innerHTML = "";
    if (flag) {
        for (var i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Juicer && productList[i].set_frec)
                productList[i].Init();
    }
    else {
        for (var i = 0; i < this.productList.length; i++)
...
var Juicer = /** @class */ (function (_super) {
    ...
    function Juicer(id, name, price, description, inStock, set_frec) {
        var _this = _super.call(this, id, name, price, description, inStock) || this;
        _this.id = id;
        _this.name = name;
        _this.price = price;
        _this.description = description;
        _this.inStock = inStock;
        _this.set_frec = set_frec;
        _this.Init();
        return _this;
    }
    Juicer.prototype.Init = function () {
        var obj = _super.prototype.Init.call(this);
        //Если можно сменить частоту, то добавляем информацию об этом в карточку
        if (this.set_frec) {
            var p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = "Можно сменить частоту";
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }
        if (document.getElementById('set_frec') == null && this.set_frec != null && this.set_frec) {
            var inp = document.createElement("input");
            inp.setAttribute("type", "checkbox");
            inp.setAttribute("id", "set_frec");
            inp.setAttribute("onclick", "CheckSetFrec(this.checked)");
            var lab = document.createElement("p");
            lab.appendChild(inp);
            lab.innerHTML += "Можно менять частоту на соковыжималке<br>";
            var div = document.getElementById('myTools');
            div.appendChild(lab);
        }
        this.Embed(obj);
...
var Saw = /** @class */ (function (_super) {
    ...
    function Saw(id, name, price, description, inStock) {
        var _this = _super.call(this, id, name, price, description, inStock) || this;
        _this.id = id;
        _this.name = name;
        _this.price = price;
...
var productList = [
    ...
    new FeltBoots(7, "Валенки1", 666.66, "Валенки великолепной работы слепого мастера Игната! В комплекте к валенкам идёт кокошник.", 2, [{ dimension: 20, color: Color.Pink, quantity: 1 },
    ...
    new Juicer(9, "Соковыжималка 3000", 666, "Выжмет все соки из твоих врагов", 1, true),
    new Saw(10, "Пилка простая", 505, "Берите в комплекте с соковыжималкой, чтобы враги помещались в неё", 1),
];
